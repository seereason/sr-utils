-- | Version of Extra.SrcLoc that pretends the getStack function is in
-- the MonadIO.  This is to ease the transition to using the new
-- backtrace mechanism in Control.Exception.Backtrace.

{-# LANGUAGE LambdaCase, OverloadedStrings, RecordWildCards #-}

module Extra.SrcLocIO
  ( getStack
  , srcloc
  , srcloccol
  , srcframe
...
getStack :: (MonadIO m, HasCallStack) => m [(String, SrcLoc)]
getStack = pure $ getCallStack callStack
...
callLocsWithInternal :: (MonadIO m, IsString s, Monoid s, HasCallStack) => ([s] -> [s]) -> m s
callLocsWithInternal f =
  f <$> (callLocList siteFormat <$> getStack) >>= \case
    [] -> pure "No call stack"
    xs -> pure $ mintercalate " <" xs

callLocsWith :: (MonadIO m, IsString s, Monoid s, HasCallStack) => ([s] -> [s]) -> m s
callLocsWith f = callLocsWithInternal (f . drop 3)

callLoc :: (MonadIO m, IsString s, Monoid s, HasCallStack) => m s
callLoc = callLocsWithInternal (take 1 . drop 3)

callLocs :: (MonadIO m, IsString s, Monoid s, HasCallStack) => m s
callLocs = callLocsWithInternal (drop 3)

-- | Function names only, no line numbers
callFnsWithInternal :: (MonadIO m, IsString s, Monoid s, HasCallStack) => ([s] -> [s]) -> m s
callFnsWithInternal f =
  f <$> (callLocList nameFormat <$> getStack) >>= \case
    [] -> pure "No call stack"
    xs -> pure $ mintercalate " <" $ xs

callFnsWith :: (MonadIO m, IsString s, Monoid s, HasCallStack) => ([s] -> [s]) -> m s
callFnsWith f = callFnsWithInternal (f . drop 3)

callFn :: (IsString s, Monoid s, HasCallStack) => IO s
callFn = callFnsWithInternal (take 1 . drop 3)

callFns :: HasCallStack => IO String
...
topLoc :: (MonadIO m, IsString s, Monoid s, HasCallStack) => m s
topLoc = compactStack <$> (take 2 <$> dropModuleFrames <$> getStack)

topLocs :: (MonadIO m, IsString s, Monoid s, HasCallStack) => Int -> m s
topLocs n = compactStack <$> (take (n + 2) <$> dropModuleFrames <$> getStack)

putStrLnLoc :: (MonadIO m, HasCallStack) => String -> m ()
putStrLnLoc msg = topLoc >>= \s -> liftIO $ putStrLn (msg <> " (" <> s <> ")")

putStrLnLocs :: (MonadIO m, HasCallStack) => Int -> String -> m ()
putStrLnLocs n msg = topLocs n >>= \s -> liftIO $ putStrLn (msg <> " (" <> s <> ")")
...
parentFunc :: (MonadIO m, HasCallStack) => String -> m String
parentFunc child = do
  s <- getStack
  case (s Fn.&
...
-- everywhere.
loc :: (MonadIO m, HasCallStack) => m SrcLoc
loc = scrubLoc <$> snd <$> here

here :: (MonadIO m, HasCallStack) => m (String, SrcLoc)
...
thisLocation :: (MonadIO m, HasCallStack, IsString s) => m s
...
-- | Pretty print the location where this appears
ici :: (MonadIO m, HasCallStack, IsString s) => m s
ici = thisLocation
...
-- "thisFunction".
thisFunction :: (MonadIO m, HasCallStack, IsString s) => m s
